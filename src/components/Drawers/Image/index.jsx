...
function DrawerImage({
  ...
  console.log(data, "checkdatatatata");
  console.log(data, "checkckckck");
  const { id, blur, block, imageSize, position, url, width } = data;
  console.log(imageSize?.value === "custom" ? width : `100%`, "cjeckImageSize");
  return (
    <div
      className={isSelected ? "block-builder selected" : "block-builder"}
      onClick={() => handleSelectBlock("image", data)}
    >
      <button
