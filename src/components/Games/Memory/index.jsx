...
function Memory({
  ...
  // console.log(data, 'dataasdadadadasd')

  const mainData = data;
  const pairs = mainData?.struct?.pairs;
  console.log(pairs, "dsddsdsdsdsdsdsdsdsdfg");
  const finalScreen = mainData?.struct?.finalScreen;
  const playground = mainData?.struct?.playground;
  ...
  // const pairList = pairs?.pairList
  console.log("object", pairList)
  ...
  }, [startGame, gameWon, nextFlip, showMemorize, data]);
  ...
  useEffect(() => {
    setCardLayout(playground?.cardLayout?.value);
  }, [playground, data]);

  console.log(shuffledList, "shuffledListshufflefdfdfdListshuffledList");
  return (
    <div
      style={{ fontSize: "unset" }}
      className={isSelected ? "block-builder selected" : "block-builder"}
      onClick={() => handleSelectBlock("memory", data)}
    >
      <button
      ...
      <div className="game_mainParent">
        <div className="editButton">
          <button
            ...
            onClick={() => handleEditModal("memory", data?.id)}
          >
            Edit
          </button>
        </div>
