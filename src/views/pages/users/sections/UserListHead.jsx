...
//           <Tabs
//           className={customization?.navType === "dark" ? "dark_tableHead" : ""}
//             value={value}
...
UserListHead.propTypes = {
  order: PropTypes.oneOf(["ACS", "DESC"]),
  orderBy: PropTypes.string,
  rowCount: PropTypes.number,
  headLabel: PropTypes.array,
  numSelected: PropTypes.number,
  onRequestSort: PropTypes.func,
  onSelectAllClick: PropTypes.func,
  onUserTypeSelection: PropTypes.func,
  userTypeSelection: PropTypes.number,
  handleSerchInputChange: PropTypes.func,
...
export default function UserListHead({
  ...
  const createSortHandler = (property) => (event) => {
    console.log(property, event, "eventeventevent")
    onRequestSort(event, property); // Trigger sort request
  };

  const [value, setValue] = useState(!userTypeSelection ? 0 : userTypeSelection - 1);
      ...
      <TableRow key="user-table-heading-row">
        {headLabel.map((headCell) => (
          ...
          >
            <TableSortLabel
              active={orderBy === headCell.id}
              direction={orderBy === headCell.id ? order.toLowerCase() : "asc"} // 👈 Just lowercase for MUI
              onClick={createSortHandler(headCell.id)}
              sx={{
                color: customization?.navType === "dark" ? "#ffffff" : "inherit",
                '&.Mui-active': {
                  color: customization?.navType === "dark" ? "#ffffff" : "#ffffff", // adjust as needed
                },
