...
import { PRODUCTS, TEMPLATES, USERS } from "app/config/endpoints";
...
const ChooseProduct = () => {
  ...
  const productListCallBack = () => {
    const _pageNo = pageNo === 0 ? 1 : pageNo;
    const roleId = !userTypeSelection ? "" : userTypeSelection;
    getProducts(
      `${PRODUCTS.LIST}?${USERS_PRM.PAGE_NO.KEY}=${_pageNo}&${USERS_PRM.ROWS_PER_PAGE.KEY}=${rowsPerPage}&${USERS_PRM.ROLE_ID.KEY}=${roleId}&${USERS_PRM.SEARCH_TEXT.KEY}=${searchText}`,
      pageNo,
      roleId
    );
  };
          ...
          {productList?.length !== 0 && (
            <Grid item xs={12} sm={4} md={3} lg={2.4}>
              <ProductCard
                ...
                icon={
                  "https://res.cloudinary.com/dwl5gzbuz/image/upload/v1728477587/kcwg2ubpjio5xirltjvg.png"
                }
                hoveredUserId={hoveredUserId}
                handleOpenMenu={handleOpenMenu}
                handlePopoverOpen={handlePopoverOpen}
