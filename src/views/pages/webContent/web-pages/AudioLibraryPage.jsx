...
import AudioLibraryCards from "../sections/audioLibrary/AudioLibraryCards";
...
function AudioLibraryPage() {
  ...
  const getContent = () => {
    getWebContent(`${WEB_CONTENT.GET}?key=AUDIO_LIBRARY&section=CATEGORY`);
  };
  ...
  return (
    <>
      ...
      <PageContainer
        ...
        heading={DEFAULT_APP_TITLE.AUDIO_LIBRARY}
      >
        <Grid
          container
          ...
          <Grid item>
            <PageBreadcrumbs
              ...
              >
                ...
                `${DEFAULT_APP_TITLE.AUDIO_LIBRARY} Category`,
              ]}
            />
          </Grid>
          <Grid item mr={1}>
            ...
            {categories.map((item, i) => {
              return (
                <Grid key={i} item xs={12} sm={4} md={3} lg={2.4}>
                  <AudioLibraryCards
                    path={`${ROUTE_SLUGS.AUDIO_LIBRARY}/${item.title.replace(/[\s/]+/g, "-").toLowerCase()}`}
                    title={item.title}
                  />
                </Grid>
              );
