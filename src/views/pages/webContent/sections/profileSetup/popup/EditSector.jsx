...
import { SECTOR, TEMPLATES_CATEGORY } from "app/config/endpoints";
...
const EditSector = ({
  ...
  const editCategory = async (data) => {
    if (preSelectedImage === null || data.icon?.length === 0) {
      ...
      if (preSelectedImage) {
        formData.append("thumbnail", preSelectedImage);
      } else {
        formData.append("thumbnail", data.icon[0]);
      }
      ...
      try {
        ...
        const {
          status,
          data: { message },
        } = await putRequest(`${SECTOR.UPDATE}/${categoryData?.id}`, formData);
  ...
  useEffect(() => {
    if (categoryData) {
      ...
      setValue("icon", categoryData?.thumbnail);
      setPreSelectedImage(categoryData?.thumbnail);
    }
