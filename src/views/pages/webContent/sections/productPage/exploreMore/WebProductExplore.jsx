...
const validationSchema = yup.object().shape({
  ...
  // imgdescription: yup.string().trim().required(`Description is required.`),
  introImage: yup
    .mixed()
...
const WebProductExplore = ({ name }) => {
  ...
  async function getWebContent(url) {
    ...
    try {
      ...
      if (success && status === RESPONSE_CODE[200]) {
        ...
        // setValue("imgdescription", imgdescription);
        setContent(description);
        // setImgContent(imgdescription);
        setValue(
          "introImage",
  ...
  const getContent = () => {
    getWebContent(`${WEB_CONTENT.GET}?key=${name}&section=EXPLORE`);
  };
              ...
              <Grid mb={4}>
                ...
                >
                  <CKEditor
                    ...
                    data={imgContent || ""}
                    onChange={(event, editor) => {
                      setValue("imgdescription", editor.getData());
                      trigger("imgdescription");
                    }}
                    onReady={(editor) => {
                      editor.editing.view.change((writer) => {
                        writer.setStyle(
...
export default WebProductExplore;
