...
const EducationBlocks = () => {
  ...
  const [imageObjDes, setImageObjDes] = useState([]);
  ...
  const deleteImageDes = async (id, index) => {
    const payload = { upload_image_response_id: id };
    try {
      dispatch({ type: REQUEST_ACTION.PROCESSING });
      const {
      ...
      if (status === RESPONSE_CODE[200]) {
        ...
        setValue(`sections[${index}].descriptionImage`, null); // Clear the icon value in the form state
        dispatch({ type: REQUEST_ACTION.SUCCESS, payload: {} });
        toast.success(message); // Show success toast
      } else {
        throw new Error(ApiErrorMessage());
  ...
  const uploadImageDes = async (event, index) => {
    const file = event.target.files[0];
    if (!file) return;
    ...
    try {
      ...
      if (status === RESPONSE_CODE[200]) {
        ...
        setValue(`sections[${index}].descriptionImage`, {
          ...data,
          size: file.size,
        }); // Update form state with uploaded image
        await trigger(`sections[${index}].descriptionImage`); // Validate the updated field
        dispatch({ type: REQUEST_ACTION.SUCCESS, payload: {} });
        toast.success(message);
      } else {
        throw new Error(ApiErrorMessage());
  ...
  async function getWebContent(url) {
    ...
    try {
      ...
      if (success && status === RESPONSE_CODE[200]) {
        ...
        const { sections } = parseValue;
        setValue("sections", sections);
        setImageObj(sections.map((sec) => sec.introImage || null));
        setImageObjDes(sections.map((sec) => sec.descriptionImage || null));
      } else {
  ...
  const getContent = () => {
    getWebContent(`${WEB_CONTENT.GET}?key=EDUCATION_PAGE&section=BLOCKS`);
  };
                ...
                <Grid item md={12} key={item.id}>
                  <Box sx={{ mb: 2 }}>
                    <Typography variant="h5">
                      {item.title ? item.title : `Section ${index + 1}`}
                    </Typography>
                  </Box>
                  <Grid mb={4}>
                    <TextField
                  ...
                  <Grid mb={4}>
                    <TextField
                      ...
                      helperText={errors?.sections?.[index]?.subtitle?.message}
                  ...
                  <Grid position="relative" mb={2}>
                    {imageObjDes[index]?.uploaded_images?.upload_image_url ? (
                      <>
                        <Tooltip title="Click to remove image" placement="top">
                          <CancelTwoToneIcon
                            sx={{
                            ...
                            onClick={() => {
                              if (
                                imageObjDes[index]?.uploaded_images
                              ...
                              trigger(`sections[${index}].descriptionImage`);
                            }}
                          />
                        </Tooltip>
                        <Avatar
                          sx={{
                            ...
                            height: "10em",
                            border: `2.5px dashed ${
                              errors[`sections[${index}].descriptionImage`]
                                ? "#ff4842"
                                : "#20A2B8"
                            }`,
                            color: `${errors[`sections[${index}].descriptionImage`] ? "#ff4842" : "#fff"}`,
                            cursor: "pointer",
                            ...
                            >
                              <AddPhotoAlternateIcon
                                sx={{
                                  ...
                                  color: `${
                                    errors[
                                      `sections[${index}].descriptionImage`
                                    ]
                                      ? "#ff4842"
                  ...
                  {errors?.sections?.[index]?.descriptionImage && (
                    <FormHelperText error>
                      {errors.sections[index].descriptionImage.message}
                    </FormHelperText>
                  )}
                    ...
                    {imageObj[index]?.uploaded_images?.upload_image_url ? (
                      <>
                        ...
                        <Avatar
                          ...
                          variant="rounded"
                          src={imageObj[index].uploaded_images.upload_image_url}
                          className="create-blog-image"
                        />
                      </>
                    ) : (
