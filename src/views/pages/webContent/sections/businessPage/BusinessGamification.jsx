...
const BusinessGamification = () => {
  ...
  async function getWebContent(url) {
    ...
    try {
      ...
      if (success && status === RESPONSE_CODE[200]) {
        ...
        const { title, sections } = parseValue;
  ...
  const getContent = () => {
    getWebContent(`${WEB_CONTENT.GET}?key=BUSINESS_PAGE&section=GAMIFICATION`);
  };
            ...
            <form onSubmit={handleSubmit(updateContent)} noValidate>
              ...
              >
                {fields.map((item, index) => (
                  <Grid item md={6} key={item.id}>
                    ...
                    <Grid mb={4}>
                      <TextField
                        {...register(`sections[${index}].url`)}
                        label="Link*"
                        disabled={loader}
                        error={!!errors?.sections?.[index]?.url}
                        helperText={errors?.sections?.[index]?.url?.message}
              ...
              >
                <Button
                  type="button"
                  onClick={() =>
                    append({ title: "", description: "", introImage: null })
                  }
                >
...
export default BusinessGamification;
